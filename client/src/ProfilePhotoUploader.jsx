...
export default function ProfilePhotoUploader({ profilePhoto, onChange }) {
    function uploadPhoto(ev) {
        const files = ev.target.files;
        const data = new FormData();
        for (let i = 0; i < files.length; i++) {
            data.append('profile-photo', files[i]);
        }
        axios.post('/profile', data, {
            headers: { 'Content-type': 'user/profile-pic' }
        }).then(response => {
    ...
    function removePhoto(ev, filename) {
        ev.preventDefault();
        onChange([...profilePhoto.filter(photo => photo !== filename)]);
    }
    return (
        <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                {profilePhoto.length > 0 && profilePhoto.map(link => (
                    <div className="h-32 flex relative" key={link}>
                        <img className="rounded-2xl w-full object-cover " src={'http://localhost:4000/profile/' + link} alt="" />
